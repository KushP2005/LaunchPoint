...
int main(void) {
    ...
    load_data(&manager);
    int counter = 0;
    pthread_t manager_thread_ex;
    pthread_t systems_thread[manager.system_array.size];

    pthread_create(&manager_thread_ex, NULL, manager_thread, &manager);
    for (int i = 0; i < manager.system_array.size; i++) {
            pthread_create(&systems_thread[i], NULL, system_thread, manager.system_array.systems[i]);
    }
    pthread_join(manager_thread_ex, NULL);
    for(int i=0; i < manager.system_array.size; i++){
        pthread_join(systems_thread[i], NULL);
    }
